...
AARRR은 원래 서비스 로그 데이터(회원가입, 클릭, 체류, 공유 이벤트) 기반 프레임워크다. 본 프로젝트의 데이터는 거래 완료 데이터로, 퍼널 진입 시점부터 이미 거래를 완료한 사용자만 존재한다. 이 구조적 제약 속에서 각 단계를 아래와 같이 재정의했다.

| 단계 | 원래 정의 | 본 프로젝트 재정의 | 한계 |
|---|---|---|---|
| Acquisition | 서비스 첫 유입 | 첫 거래 발생 유저 | 신규 가입과 첫 거래를 동일시 |
| Activation | 서비스에서 가치를 처음 느끼는 순간 | 첫 거래 후 30일 내 재거래 | 재거래는 Retention에 더 가까움 |
| Retention | 반복 재방문 | 첫 거래 다음 연도 재거래 | Activation 미통과자 포함, 1년 기준의 논리적 근거 부족 |
| Revenue | 실제 매출 전환 | 유저별 총 거래금액 | 거래 데이터 특성상 퍼널 전 단계에서 이미 발생 중 |
| Referral | 추천·공유 행동 | 미측정 | 거래 데이터에 추천 정보 없음 |

Activation을 30일 내 재거래로 정의한 것은 행동 반복성의 첫 신호를 측정하려는 의도였으나, 엄밀히는 Retention의 초기 형태에 가깝다. 이 한계를 인지한 상태에서, 데이터 제약 내 최선의 근사치로 채택했다.
